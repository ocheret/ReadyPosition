...
/** Interface for maps with _KeyType_ keys and Generic values */
public interface Map_KeyName_G<V> extends Itemizable {
     ...
     *            getEntry(_KeyType_ key) and check for a result of -1
     *            in order to avoid checking for this exception. If
     *            the result is not -1, then _ValueType_
     *            getEntryValue(int entry) can be called with the
     *            result of int getEntry(_KeyType_ key) to efficiently
     *            retrieve the value.
     */
    V get(_KeyType_ key);
     ...
     * @return the value to which the specified key is mapped
     */
    V get(_KeyType_ key, V missingValue);
     ...
     * 
     * @param key the key of the mapping to be removed from this map if is is
     *            present.
     */
